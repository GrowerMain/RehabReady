package com.example.rehabready.Helpers;
...
import com.example.rehabready.SimpleClasses.Note;
            ...
            if (count != 0) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                        lt = LocalTime.parse(cursor1.getString(cursor1.getColumnIndexOrThrow(DRUG_TIME)));
                    }
        ...
        while (!meds.isAfterLast()) {
            ...
            if (count != 0) {
                ...
                for (int i = 0; i < count; i++) {
                    LocalTime lt = null;
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        lt = LocalTime.parse(cursor.getString(cursor.getColumnIndexOrThrow(DRUG_TIME)));
                    }
    ...
    public void updateMedication(Medication medication) {
        ...
        if (medication.getTimes().length > 0) {
            ...
            for (int i = 0; i < maxIndex; i++) {
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                    timeAsString = medication.getTimes()[i].toLocalTime().toString() + ":00";
                }
            ...
            if (diff > 0) {
                for (int i = maxIndex; i < medication.getTimes().length; i++) {
                    String timeAsString = null;
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                        timeAsString = medication.getTimes()[i].toLocalTime().toString();
                    }
